import Dashboard from './dashboard/page'; // Move old home to dashboard
import Onboarding from './onboard/page';
import { useState } from 'react';

export default function Home() {
  const [view, setView] = useState<'landing' | 'dashboard' | 'onboard'>('landing');
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[500px] bg-blue-600/20 blur-[120px] rounded-full z-0" />
      ...
      <div className="relative z-10 max-w-4xl text-center space-y-8">
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-center pt-8">
          <button 
            ...
            className="px-8 py-4 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-lg text-lg transition-all shadow-[0_0_30px_rgba(37,99,235,0.4)] hover:shadow-[0_0_50px_rgba(37,99,235,0.6)]"
          >
            Start Free Trial
          </button>
          <button 
            onClick={() => setView('dashboard')}
            className="px-8 py-4 glass-panel hover:bg-white/10 text-white font-semibold rounded-lg text-lg transition-all"
          >
            Login to Dashboard
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-16 text-left">
          <FeatureCard 
            ...
            desc="Answers every call instantly, 365 days a year. No holidays, no sick days."
          />
          <FeatureCard 
            title="Smart Qualification" 
            desc="Distinguishes emergencies from routine calls. Books appointments automatically."
          />
          <FeatureCard 
            title="90%+ Profit Margins" 
            desc="Built on self-hosted infrastructure. Cheaper than human operators."
          />
        </div>
      </div>
    </div>
